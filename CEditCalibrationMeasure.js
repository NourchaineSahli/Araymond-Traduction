var CEditCalibrationMeasure = (function ($) {
    ...
    $.extend(CEditCalibrationMeasure.prototype, {
        ...
        processData: function (aResult) {
            ...
            var measureSeries = aResult.Series.find(s => s.SeriesTypes && (s.SeriesTypes.sRawName === "Measure" || s.SeriesTypes.sName === "Mesure"));
            var minTolSeries = aResult.Series.find(s => s.SeriesTypes && s.SeriesTypes.sName === "Tol - Min");
            var maxTolSeries = aResult.Series.find(s => s.SeriesTypes && s.SeriesTypes.sName === "Tol - Max");
            var dateSeries = aResult.Series.find(s => s.SeriesTypes && (s.SeriesTypes.sRawName === "Real Date" || s.SeriesTypes.sName === "Date réelle"));
         ...
         */
        onValidate: function () {
            var measureInput = document.querySelector('input[id^="newMeasure"]');
            var realDateInput = document.querySelector('input[id^="newRealDate"]');


            var measureValue = measureInput ? measureInput.value : '';
            var realDateValue = realDateInput ? realDateInput.value : '';
            ...
            var tolMinInput = document.querySelector('input[id^="prevTolMin"]');
            var tolMaxInput = document.querySelector('input[id^="prevTolMax"]');

            this.newCalibrationData = {
                measure: measureValue,
                realDate: realDateValue,
                tolMin: tolMinInput ? tolMinInput.value : '',
                tolMax: tolMaxInput ? tolMaxInput.value : ''
            };
        ...
        updateDataAfterValidation: function () {
            ...
            var timeSeries = newData.Series.find(s => s.SeriesTypes && (s.SeriesTypes.ID === this.idSeriestypesTime));
            if (timeSeries) {
            ...
            var measureSeries = newData.Series.find(s => s.SeriesTypes && s.SeriesTypes.ID === this.idSeriestypesMeasure);
            if (measureSeries && newValues.measure) {
                measureSeries.Values.push({
                    "ID": ++maxId,
                    "sName": newValues.measure,
                    "sType": "tvtNumerical"
                });
            }

            var tolMinSeries = newData.Series.find(s => s.SeriesTypes && s.SeriesTypes.ID === this.idSeriestypesTolMin);
            if (tolMinSeries && newValues.tolMin) {
                tolMinSeries.Values.push({
                    "ID": ++maxId,
                    "sName": newValues.tolMin,
                    "sType": "tvtNumerical"
                });
            } else if (tolMinSeries) {
                tolMinSeries.Values.push({ "ID": ++maxId, "sName": "0", "sType": "tvtNumerical" });
            }

            var tolMaxSeries = newData.Series.find(s => s.SeriesTypes && s.SeriesTypes.ID === this.idSeriestypesTolMax);
            if (tolMaxSeries && newValues.tolMax) {
                tolMaxSeries.Values.push({
                    "ID": ++maxId,
                    "sName": newValues.tolMax,
                    "sType": "tvtNumerical"
                });
            } else if (tolMaxSeries) {
                tolMaxSeries.Values.push({ "ID": ++maxId, "sName": "0", "sType": "tvtNumerical" });
            }

            var realDateSeries = newData.Series.find(s => s.SeriesTypes && (s.SeriesTypes.ID === this.idSeriestypesRealDate));
            if (realDateSeries && formattedDate) {
                realDateSeries.Values.push({
                    "ID": ++maxId,
                    "sName": formattedDate,
                    "sType": "tvtUndefined"
                });
